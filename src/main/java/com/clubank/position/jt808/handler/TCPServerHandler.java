	...
	private void processPackageData(PackageData packageData) {
		...
		// 位置信息汇报 ==> 平台通用应答
			...
			try {
				...
				// 将数据传输到业务服务器
				HttpUtil.post(PropertyUtil.getProperty("url"), JSON.toJSONStringWithDateFormat(gpsLocations,
		...
		// 其他情况
			...
			log.error(">>>>>>[未知消息类型],terminalNo={},msgId={},package={}", header.getTerminalNo(), header.getMsgId(),
					packageData);
		}
	}
